...
function listRedirects()	{
		...
		if (is_array($redirects)) {
			foreach ($redirects as $redirect) {
				...
				} elseif ($redirect['endtime'] > 0) {
					$outTable .= date($GLOBALS['TYPO3_CONF_VARS']['SYS']['ddmmyy'].' '.$GLOBALS['TYPO3_CONF_VARS']['SYS']['hhmm'], $redirect['endtime']) ;
				} else {
					$outTable .= $GLOBALS['LANG']->getLL('never', TRUE) ;
				}
				$outTable .=
						'</td>' .
						'<td nowrap="nowrap">'.$redirect['sys_language_uid'].'&nbsp;&nbsp;</td>' .
		...
		<table border="0" cellpadding="0" cellspacing="0" class="typo3-dblist">
			<tr class="t3-row-header">
				...
				<td >' . $GLOBALS['LANG']->getLL('language', TRUE) . '</td>
				<td>' . $GLOBALS['LANG']->getLL('hits_count', true) . '</td>
                <td>' . $GLOBALS['LANG']->getLL('last_called', true) . '</td>
                <td>' . $GLOBALS['LANG']->getLL('last_referer', true) . '</td>
